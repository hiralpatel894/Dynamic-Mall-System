			...
			<div id="page-wrapper">
				...
				<div id="title-breadcrumb-option-demo" class="page-title-breadcrumb">
					<div class="page-header pull-left">
						<div class="page-title">Seller profile</div>
					</div>
						...
						<div class="col-lg-12">
							<div class="portlet box portlet-yellow">
								<div class="portlet-header">
									<div class="caption">Seller profile </div>
									<div class="tools">
								...
								<%@taglib prefix="c" uri="http://java.sun.com/jstl/core_rt" %>
									<form action="<%=request.getContextPath() %>/ProfileController?flag=updateprofile" method="post" class="form-validate">
										<c:forEach items="${sessionScope.editpro }" var="x">
										<div class="form-group">
											<label for="dd1" class="control-label">Country Name</label>
											<select id="dd1" name="country_name" class="form-control required">
											
											<c:forEach items="${sessionScope.country}" var="i" >
											<c:if test="${i.country_Id eq x.countryVO.country_Id}">
											<option value="${i.country_Id}" selected="selected">${i.country_Name}</option>					
						  					<c:if test="${i.country_Id ne x.countryVO.country_Id}">
										...
										<div class="form-group">
											<label for="dd2" class="control-label">State Name</label>
											<select id="dd2" name="state_name" class="form-control required">
											
											<c:forEach items="${sessionScope.state}" var="j" >
											<c:if test="${j.state_Id eq x.stateVO.state_Id }">
											<option value="${j.state_Id}" selected="selected">${j.state_Name}</option>
											<c:if test="${j.state_Id ne x.stateVO.state_Id }">
										...
										<div class="form-group">
											<label for="dd3" class="control-label">City Name</label>
											<select id="dd3" name="city_city" class="form-control required">
											
											<c:forEach items="${sessionScope.city}" var="k" >
											<c:if test="${k.city_Id eq x.cityVO.city_Id}">
											<option value="${k.city_Id}" selected="selected">${k.city_City}</option>											
						  					<c:if test="${k.city_Id ne x.cityVO.city_Id}">
											<option value="${k.city_Id}" >${k.city_City}</option>											
						  					</c:if>
						  					</c:if>
						  					</c:forEach>	
										...
										<div class="form-group">
											<label for="dd4" class="control-label">Area Name</label>
											<select id="dd4" name="area_name" class="form-control required">
											
											<c:forEach items="${sessionScope.area}" var="m" >
											<c:if test="${m.area_Id eq x.areaVO.area_Id }">
											<option value="${m.area_Id}" selected="selected">${m.area_Name}</option>
											<c:if test="${m.area_Id ne x.areaVO.area_Id }">
											<option value="${m.area_Id}" >${m.area_Name}</option>
											</c:if>
											</c:if>
						  					</c:forEach>	
											...
											<div class="form-group">
											<label for="" class="control-label">Pin code</label><input
												type="number" name="pincode" value="${x.pin_Code }" class="form-control required" />
										</div>
											...
											<label for="" class="control-label">Mobile number </label><input
												type="number" size="12" name="mobile" value="${x.mobile}" class="form-control required" />
										</div>
										
										<div class="form-group">
											<label for="" class="control-label">Phone number</label><input
												type="number" name="phone" value="${x.phone_Number }" class="form-control required" />
										</div>
										
										
										<div class="form-group">
										<label for="" class="control-label">Temp Address</label>
											<textarea rows="4" name="temp_addrs" class="form-control required">${x.temp_Addr}</textarea>
										</div>
										
										<div class="form-group">
										<label for="" class="control-label">Permanent Address</label>
											<textarea rows="4" name="permnt_addrs" class="form-control required">${x.perm_Addr }</textarea>
										</div>
										
										<div class="form-group">
											<label for="" class="control-label">Shop Name</label><input
												type="text" name="shop_name" value="${x.shop_Name }" class="form-control required" />
										</div>
										
										<div class="form-group">
										<label for="" class="control-label">Shop Description</label>
											<textarea rows="4" name="shop_desc"	class="form-control required">${x.shop_Desc }</textarea>
										</div>
										
										<input type="hidden" name="flag" value="updateprofile">
										<input type="hidden" name="profileId" value="${x.profile_Id}">
