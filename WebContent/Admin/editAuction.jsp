						...
						<div class="col-lg-12">
							<div class="portlet box portlet-yellow">
								<div class="portlet-header">
									<div class="caption">Add Auction </div>
									<div class="tools">
								...
								<c:forEach items="${sessionScope.set}" var="k">
									<form action="<%=request.getContextPath() %>/AuctionController?flag=update" method="post" class="form-validate">
										
										<div class="form-group">
										<input type="hidden" value="${k.auction_Id}" name="Id"> 
											<label for="dd1" class="control-label">Category Name</label>
											<select id="dd1" name="cat_Id" class="form-control required">
											<option value="">choose</option>	
																									  						
											<c:forEach items="${sessionScope.category}" var="i" >
											<c:if test="${i.cat_Id eq k.categoryVO.cat_Id}">
											<option value="${i.cat_Id}" selected="selected">${i.cat_Name }</option>
											</c:if>
											<c:if test="${i.cat_Id ne k.categoryVO.cat_Id}">
										...
										<div class="form-group">
											<label for="dd2" class="control-label">Subcategory Name</label>
											<select id="dd2" name="subcat_Name" class="form-control required">
											<option value="">choose</option>	
											
											<c:forEach items="${sessionScope.subcat1}" var="s" >
											<c:if test="${s.subcat_Id eq k.subcategoryVO.subcat_Id}">
											<option value="${s.subcat_Id}" selected="selected">${s.subcat_Name }</option>
											</c:if>
											<c:if test="${s.subcat_Id ne k.subcategoryVO.subcat_Id}">
										...
										<div class="form-group">
											<label for="dd3" class="control-label">Product Name</label>
											<input type="text" name="product_Name" value="${k.product_Name}" class="form-control required" />
										</div>
										
										<div class="form-group">
											<label for="dd3" class="control-label">Product Description</label>
											
											<textarea rows="4" name="product_Desc"
												class="form-control required">${k.product_Desc}</textarea>
										</div>
										
										<c:forEach items="${sessionScope.map}" var="s">
											<img alt="" src="<%=request.getContextPath() %>/doc/${s.path}" width="100px" height="100px"/>
										</c:forEach>	
										
										
										<div class="form-group">
											<label for="dd3" class="control-label">Logo</label>
											<iframe src="fileUpload1.jsp" width="100%" height="250" name="frames"> </iframe>								
										</div>
										
										<div class="form-group">
											<label for="dd3" class="control-label">Product Brand</label>
											<input type="text" name="product_Brand"  value="${k.product_Brand}"  class="form-control required" />
										</div>
										...
										<div class="form-group">
											<label for="dd3" class="control-label">Product Images</label>
											<iframe src="fileupload.jsp" width="100%" height="250" name="frames"> </iframe>
										</div>
										
											<div class="form-group">
											<label for="" class="control-label">Arrival Time</label><input
												type="text" name="arrival_time"  value="${k.arrival_time}" class="form-control required" />
										</div>
											
											<div class="form-group">
											<label for="" class="control-label">Departure Time</label><input
												type="text" name="departure_time" value="${k.departure_time}" class="form-control required" />
										</div>
											
											
											
											<div class="form-group">
											<label for="" class="control-label">Minimum Bid Amount </label><input
												type="text" size="12" name="min_bid_amt" value="${k.min_bid_amt}" class="form-control required" />
										</div>
