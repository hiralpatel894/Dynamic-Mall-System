			...
			<div id="page-wrapper">
				...
				<div id="title-breadcrumb-option-demo" class="page-title-breadcrumb">
					<div class="page-header pull-left">
						<div class="page-title">CITY FORM</div>
					</div>
						...
						<div class="col-lg-12">
							<div class="portlet box portlet-yellow">
								...
								<%@taglib prefix="c" uri="http://java.sun.com/jstl/core_rt" %>
									<form action="<%=request.getContextPath() %>/CityController?flag=UpdateCity" method="post" class="form-validate">
										<c:forEach items="${sessionScope.edit}" var="i">
										<div class="form-group">
											<label for="dd1" class="control-label">Country</label>
											<select
												id="dd1" name="city_country" class="form-control required"><option
													value="">nothing</option>
												<option value="">choose one</option>																								 
												<c:forEach items="${sessionScope.country1}" var="x">
												<c:if test="${x.country_Id eq i.countryVO.country_Id}">
													<option value="${x.country_Id}" selected="selected">${x.country_Name}</option>
													</c:if>
												     <c:if test="${x.country_Id ne i.countryVO.country_Id}">
										...
										<div class="form-group">
											<label for="dd1" class="control-label">State</label>
											<select id="dd1" name="city_state" class="form-control required">
											<option value="">choose one</option>										
											<c:forEach items="${sessionScope.state1}" var="y">
											<c:if test="${y.state_Id eq i.stateVO.state_Id}">
											<option value="${y.state_Id }" selected="selected">${y.state_Name}</option>
											</c:if>
											<c:if test="${y.state_Id ne i.stateVO.state_Id}">
											<option value="${y.state_Id }">${y.state_Name}</option>
											</c:if>
											</c:forEach>	
												</select>
										</div>
										<div class="form-group">
											<label for="" class="control-label">City Name</label><input
												type="text" name="city_Name" value="${i.city_Name}" class="form-control required" />
										</div>
										
										<div class="form-group">
													<label>City Description</label>
													<textarea rows="4" class="form-control" name="city_Des">${i.city_Des}</textarea>
												</div>
										
										<input type="hidden" name="flag" value="UpdateCity">
										<input type="hidden" name="cityId" value="${i.city_Id}">
										
										<button type="submit" class="bt	n btn-success">Update</button>
										&nbsp;
